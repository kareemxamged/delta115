...
import {
    ...
    Phone, Calendar, Save, Edit2, Lock, ChevronDown
} from 'lucide-react';
    ...
    return (
        <div className={styles.card}>
            ...
            <div className={styles.cardHeader}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                    ...
                    <button
                        ...
                        {isEditing ? <><Save size={16} /> {isSubmitting ? 'Saving…' : 'Save Changes'}</> : <><Edit2 size={16} /> Edit Profile</>}
                    </button>
                </div>
            </div>
            ...
            <div className={styles.formGrid}>
                ...
                <Field label="Major" icon={<BookOpen size={14} />} error={errors.major?.message}>
                    <div className={styles.inputWrapper}>
                        <select {...register('major')} disabled={!isEditing}
                            className={inputCls(!!errors.major)} style={{ appearance: 'none' }}>
                        ...
                        {isEditing
                            ? <ChevronDown size={14} className={styles.lockIcon} />
                            : <Lock size={14} className={styles.lockIcon} />}
                    </div>
                </Field>

                <Field label="Level / Year" icon={<BarChart2 size={14} />} error={errors.level?.message}>
                    <div className={styles.inputWrapper}>
                        <select {...register('level')} disabled={!isEditing}
                            className={inputCls(!!errors.level)} style={{ appearance: 'none' }}>
                ...
                <Field label="Mobile Number" icon={<Phone size={14} />} error={errors.mobile?.message}>
                    <div className={styles.inputWrapper}>
                        <input {...register('mobile')} disabled={!isEditing} type="tel"
                            className={inputCls(!!errors.mobile)} placeholder="+201234567890" />
                        {!isEditing && <Lock size={14} className={styles.lockIcon} />}
                    </div>
                </Field>
